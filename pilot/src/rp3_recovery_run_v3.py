"""RP3-MR1 final rerun: frozen RP3 procedure with immutable generations."""
import argparse
import os
...
from rp3_durable_v3 import write_json, validate, sync_directory


class Run(rp3.Run):
    def __init__(self, args):
        super().__init__(args)
        self.report.update(experiment='RP3-MR1-V3', original_experiment='BLOCKED',
    ...
    def checkpoint(self):
        ...
        # Checkpoints are immutable generations; no older checkpoint can
        # overwrite the final report after process exit.
        self.generation += 1
        write_json(self.directory/f'checkpoint-{self.generation:04d}.json', self.report)
    ...
    def finish(self):
        ...
        super().finish()
        self.journal.close()
        # Publish one immutable final report and hard-link report.json to it.
        # The marker is written only after two complete validations.
        self.report['durability_check'] = 'immutable-generation-hardlink-v3'
        final_path = self.directory/'report.final.json'
        write_json(final_path, self.report)
        if self.args.output.exists(): self.args.output.unlink()
        os.link(final_path, self.args.output)
        sync_directory(self.directory)
        assert self.args.output.read_bytes() == final_path.read_bytes()
