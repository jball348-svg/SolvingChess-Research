"""Bounded RP3-MR1 immutable-generation rerun. Earlier attempts are never overwritten."""
import argparse
import datetime
import json
import os
...
from rp3_durable_v3 import write_json, validate

OLD = ROOT/'pilot/evidence/rp3'
FIRST = ROOT/'pilot/evidence/rp3-recovery'
SECOND = ROOT/'pilot/evidence/rp3-recovery-v2'
...
def freeze_update(base, rung, source, parent, output):
    update = dict(schema=1, experiment='RP3-MR1-V3', rung=rung,
...
def inherit_r2(output, source):
    ...
    frozen = dict(schema=1, experiment='RP3-MR1-V3', rung='r2', usable_at='r3',
...
def prepare(output):
    ...
    second_source = SECOND/'source-freeze.json'
    second_execution = SECOND/'execution-complete.json'
    assert second_source.exists() and second_execution.exists()
    first_execution = FIRST/'execution-complete.json'
    assert first_source.exists() and first_execution.exists()
    sources = [
        'pilot/src/rp3_durable_v3.py', 'pilot/src/rp3_recovery_run_v3.py',
        'pilot/src/rp3_recovery_measure_v3.py', 'pilot/src/rp3_recovery_campaign_v3.py',
        'pilot/tests/test_rp3_recovery.py', 'pilot/RP3_RECOVERY.md',
        'pilot/RP3_RECOVERY_V2.md', 'pilot/RP3_RECOVERY_V3.md']
    frozen = dict(old, experiment='RP3-MR1-V3', frozen_utc=now(),
                  ...
                  second_attempt_source_freeze_sha256=file_hash(second_source),
                  second_attempt_execution_sha256=file_hash(second_execution),
                  amendment_sha256=file_hash(ROOT/'pilot/RP3_RECOVERY.md'),
                  rerun_amendment_sha256=file_hash(ROOT/'pilot/RP3_RECOVERY_V3.md'),
...
def run(output, source, commit):
    ...
    second_cpu = sum(e['cpu_s'] for e in read(SECOND/'sequence.json')) if (SECOND/'sequence.json').exists() else 0
    ...
    def execute(mode, rung, arm, repeat, diagnostic='primary'):
        ...
        if directory.exists():
            ...
            return
        command = [sys.executable, str(ROOT/'pilot/src/rp3_recovery_measure_v3.py'),
                   '--output', str(directory/'envelope.json'), '--', sys.executable,
                   str(ROOT/'pilot/src/rp3_recovery_run_v3.py'), mode, '--rung', rung,
        ...
        validate(directory, 24 if mode == 'acquire' else 16)
        if old_cpu + first_cpu + second_cpu + sum(e['cpu_s'] for e in sequence) > 4*3600:
    ...
    try:
        ...
        write_json(output/'execution-complete.json', dict(
            experiment='RP3-MR1-V3', status='COMPLETE', processes=len(sequence),
